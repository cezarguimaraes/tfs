if not Battlefield then
	Battlefield = {
		...
		wall = {
			...
			top = Position(94, 124, 7),
			bottom = Position(94, 129, 7)
		},
	...
	function Battlefield:Close(winner)
		...
		if winner then
			Game.broadcastMessage(string.format("Time %s venceu o evento!", self.teams[winner].name))
		else
			Game.broadcastMessage("Ocorreu um empate.")
		end
	...
	function Battlefield:onJoin(player)
		...
		Game.broadcastMessage(string.format("%s entrou no evento battlefield!", info.name))
	...
	function Battlefield:onLeave(player)
		...
		Game.broadcastMessage(string.format("%s saiu do evento battlefield!", info.name))
